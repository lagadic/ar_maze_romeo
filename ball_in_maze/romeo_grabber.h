...
void *grab_compute_pose(void * arg)
{
  ...
  try
  {
    ...
    for (unsigned int i=0; i<arg_struct.argc; i++) {
      ...
      else if (std::string(arg_struct.argv[i]) == "--fr")
        opt_language_english = false;
      else if (std::string(arg_struct.argv[i]) == "--help") {
        std::cout << "Usage: " << arg_struct.argv[0] << "[--box-name] [--data-folder] [--no-controll] [--fr]" << std::endl;
    ...
    //0 - UP -x
    M_offset.buildFrom(0.0, 0.0, 0.0, -d_r-0.07, 0.0, 0.0) ;
    ...
    std::string opt_name_file_color_target1_l = opt_name_file_color_target_path + chain_name[0] +"/color.txt";
    std::string opt_name_file_color_target1_r = opt_name_file_color_target_path + chain_name[1] +"/color.txt";
    ...
    //    AL::ALValue angles_head      = AL::ALValue::array(vpMath::rad(-4.6), vpMath::rad(15.0), vpMath::rad(6.3), vpMath::rad(0.0), vpMath::rad(6.0) , vpMath::rad(9.8), vpMath::rad(6.0) , vpMath::rad(9.8)  );
    ...
    while(1)
    {
      ...
      if (ret)
      {
        ...
        if (s == "i")
        {
          t_tracker.setForceDetection();
          std::cout << "Force Detection" << std::endl;
        }
        if (s == "m")
        {
          manual_mode = true;
      ...
      if (state > FindTemplate && state < WaitPreGrasp && state != NoControll)
      {
      ...
      if (state >= FindTemplate)
      {
        ...
        if (status_template_tracker ) { // display the tracking results
          ...
          static bool template_found = false;
          if (!template_found)
          {
            if(opt_language_english)
              tts.post.say("\\emph=2\\ \\rspd=100\\ I see a picture \\eos=1\\ \\pau=150\\ with my robot friends  \\pau=80\\ \\emph=2\\ R 2 D 2 and \\emph=2\\ C 3 P O \\eos=1\\  I have a brilliant idea!");
            else
              tts.post.say(" \\emph=2\\ Bien, Je vois une photo. \\eos=1\\  \\wait=800\\  J'ai une \\emph=2\\  idée \\eos=1\\" );
            template_found = true;
          }
      ...
      if (state == MoveHead)
      {
        ...
        if (first_time)
        {
          ...
          AL::ALValue angles_head = AL::ALValue::array(vpMath::rad(-6.9), vpMath::rad(17.0), vpMath::rad(6.3), vpMath::rad(0.0), vpMath::rad(6.0) , vpMath::rad(9.8), vpMath::rad(6.0) , vpMath::rad(9.8)  );
          float fractionMaxSpeed = 0.03f;
          robot.getProxy()->setAngles(jointHeadNames_tot, angles_head, fractionMaxSpeed);
        }
        if (click_done && button == vpMouseButton::button1 ) {
          state = FindTemplate;
      ...
      if (state == FindTemplate &&  status_template_tracker )
      {
        if (click_done && button == vpMouseButton::button1 ) {
          ...
          if(opt_language_english)
            tts.post.say("\\emph=2\\First of all I have to calibrate my right hand!");
          else
            tts.post.say(" \\emph=2\\ Mais tout d'abord je dois calibrer ma main droite." );

          angles_head      = AL::ALValue::array(vpMath::rad(-18.8), vpMath::rad(21.9), vpMath::rad(11.8), vpMath::rad(0.0), vpMath::rad(6.0) , vpMath::rad(9.8), vpMath::rad(6.0) , vpMath::rad(9.8)  );
      ...
      if (state == CalibrateRigthArm &&  status_template_tracker && status_hand_tracker[index_hand] )
      {
        ...
        if (click_done && button == vpMouseButton::button1 ) {
          ...
          {
            if(opt_language_english)
              tts.post.say( "\\emph=2\\ Now I have to calibrate my left hand!");
            else
              tts.post.say("\\emph=2\\OK \\eos=1\\ \\wait=600\\ C'est maintenant à ma main gauche." );
            template_found = true;
          }
          // Small Plate
          AL::ALValue angles_head      = AL::ALValue::array(vpMath::rad(4.7), vpMath::rad(21.9), vpMath::rad(11.9), vpMath::rad(0.0), vpMath::rad(6.0) , vpMath::rad(9.8), vpMath::rad(6.0) , vpMath::rad(9.8)  );

          float fractionMaxSpeed  = 0.03f; //0.01
          robot.getProxy()->setAngles(jointHeadNames_tot, angles_head, fractionMaxSpeed);
      ...
      if (state == CalibrateLeftArm && status_template_tracker && status_hand_tracker[index_hand] )
      {
        ...
        if (click_done && button == vpMouseButton::button1 ) {
          ...
          if(opt_language_english)
            tts.post.say(" \\emph=2\\ Perfect, We are almost ready!");
          else
            tts.post.say(" \\emph=2\\ C'est parfait, on y est presque." );

          box_Ve_Arm[index_hand].buildFrom(box_Me_Arm);
          state = WaitPreGrasp;
          click_done = false;
          //AL::ALValue names_head     = AL::ALValue::array("NeckYaw","NeckPitch","HeadPitch","HeadRoll","LEyeYaw", "LEyePitch","LEyeYaw", "LEyePitch" );
          AL::ALValue angles_head      = AL::ALValue::array(vpMath::rad(-6.9), vpMath::rad(13.4), vpMath::rad(9.1), vpMath::rad(0.0), vpMath::rad(6.0) , vpMath::rad(9.8),vpMath::rad(6.0) , vpMath::rad(9.8)  );
          float fractionMaxSpeed  = 0.03f;
          robot.getProxy()->setAngles(jointHeadNames_tot, angles_head, fractionMaxSpeed);
        }
      }
      ...
      if (state == VSBox)
      {
        ...
        {
          if(opt_language_english)
            tts.post.say("I will use my hands to move the tray in a convenient position!");
          else
            tts.post.say("\\emph=2\\ Maintenant j'utilise mes deux bras pour déplacer le plateau dans une position plus pratique." );
          init_pose = true;
        }
        ...
        if(status_template_tracker )
        {
          ...
          if (! grasp_servo_converged[0]) {
            ...
            if ( (sqrt(t_error_grasp.sumSquare()) < error_t_treshold) && (theta_error_grasp < vpMath::rad(2)) && first_time_box_pose)
            {
              ...
              static bool start_game_bool = false;
              if (!start_game_bool)
              {
                if(opt_language_english)
                  tts.post.say(" \\emph=2\\  Now I am ready to start the game!");
                else
                {
                  tts.post.say(" \\emph=2\\ Maintenant je suis prêt à commencer le jeu." );
                }
              ...
              if (click_done && button == vpMouseButton::button1)
              {
                ...
                if(opt_language_english)
                {
                  tts.post.say(" \\emph=2\\ Let's start \\pau=4000\\ ");
                  tts.post.say(" \\rspd=80\\ The aim here is to roll the ball till the end of the maze  \\eos=1\\  \\pau=2000\\ I need to focus \\eos=1\\  \\pau=3000\\ This kind of task requires a perfet coordination of my hands \\eos=1\\ \\pau=5000\\  \\emph=2\\ We are almost there \\eos=1\\ \\pau=3300\\ \\rspd=110\\ \\emph=2\\ Yes! \\emph=2\\ I did \\emph=2\\ it! ");
                }
                else
                {
                  phraseToSay = "\\rspd=86\\ \\emph=2\\ Le but est d'apporter la balle à la fin du labyrinthe \\eos=1\\  \\wait=1000\\ Je dois me concentrer\\eos=1\\  \\wait=300\\ cette tâche exige une coordination parfaite de mes deux bras \\eos=1\\";
            ...
            if (!first_time_box_pose && !manual_mode)
            {
